...
class _FacultyAttendanceScreenState extends State<FacultyAttendanceScreen> {
  ...
  Future<void> _saveAttendance() async {
    ...
    try {
      ...
      final existing = await FirebaseFirestore.instance.collection('faculty_attendance').where('date', isEqualTo: dateStr).get();
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: Padding(
                      ...
                      child: Row(
                        children: [
                          CircleAvatar(backgroundColor: _getStatusColor(status).withOpacity(0.2), child: Text((faculty['name'] ?? '?')[0].toUpperCase(), style: TextStyle(color: _getStatusColor(status), fontWeight: FontWeight.bold))),
                          const SizedBox(width: 12),
                          SizedBox(width: 120, child: Text(faculty['name'] ?? '', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14), overflow: TextOverflow.ellipsis)),
                          const SizedBox(width: 8),
                          SizedBox(width: 150, child: Text('Subject: ${faculty['subject'] ?? 'N/A'}', style: const TextStyle(fontSize: 13), overflow: TextOverflow.ellipsis)),
                          const Spacer(),
