...
class _MarkAttendanceScreenState extends State<MarkAttendanceScreen> {
  ...
  Future<void> _loadStudents() async {
    ...
    try {
      ...
      // If attendance exists, load the previous status
      if (existingAttendance.docs.isNotEmpty) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('⚠️ Attendance already marked for this date. You can update it.'),
  ...
  Future<void> _saveAttendance() async {
    ...
    bool? confirm = await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Save attendance for ${_students.length} students on ${_formatDate(_selectedDate)}?',
        ),
        actions: [
          TextButton(
    ...
    try {
      ...
      // Delete existing attendance for this date/batch first
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('✅ Attendance saved for ${_students.length} students!'),
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  _buildStatusChip('Late', _countByStatus('late'), Colors.orange),
                  const Spacer(),
                  TextButton.icon(
          ...
          Expanded(
            child: _isLoading
                ...
                : _selectedBatchId == null
                    ? const Center(
                        child: Text('Select a batch to view students',
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return Card(
                                ...
                                child: ListTile(
                                  ...
                                  trailing: _buildStatusToggle(student['id'], status),
                                ),
                              );
                            },
                          ),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text('$count $label', style: TextStyle(color: color, fontWeight: FontWeight.bold)),
        ],
      ),
    );
  }
  ...
  Widget _buildStatusToggle(String studentId, String currentStatus) {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        _buildStatusButton(studentId, 'present', 'P', Colors.green, currentStatus),
        const SizedBox(width: 4),
        _buildStatusButton(studentId, 'absent', 'A', Colors.red, currentStatus),
        const SizedBox(width: 4),
